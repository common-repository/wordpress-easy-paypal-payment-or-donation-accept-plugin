...
use TTHQ\WPAPP\Lib\PayPal\PayPal_Bearer;
use TTHQ\WPAPP\Lib\PayPal\PayPal_PPCP_Config;
use TTHQ\WPAPP\Lib\PayPal\Onboarding\PayPal_PPCP_Onboarding;
use TTHQ\WPAPP\Lib\PayPal\Onboarding\PayPal_PPCP_Onboarding_Serverside;

class WPAPP_PPCP_settings_page
{

	private $settings;

	private $ppcp_connection_subtab_url = "options-general.php?page=wordpress-easy-paypal-payment-or-donation-accept-plugin%2Fadmin%2Fwpapp_admin_menu.php&action=ppcp-settings&subtab=api-connection";
	private $ppcp_api_creds_subtab_url = "options-general.php?page=wordpress-easy-paypal-payment-or-donation-accept-plugin%2Fadmin%2Fwpapp_admin_menu.php&action=ppcp-settings&subtab=api-credentials";
	private $ppcp_button_subtab_link = "options-general.php?page=wordpress-easy-paypal-payment-or-donation-accept-plugin%2Fadmin%2Fwpapp_admin_menu.php&action=ppcp-settings&subtab=button-appreance";
	...
	public function handle_ppcp_settings_tab()
	{
		...
		do_action('wpapp_ppcp_settings_menu_tab_start');

		//Documentation link
		$ppcp_documentation_link = "https://www.tipsandtricks-hq.com/wordpress-easy-paypal-payment-or-donation-accept-plugin-120#wpapp-usage";
		echo '<div class="wpapp-grey-box">';
		echo '<p>';
		...
		echo '&nbsp;' . '<a href="' . esc_url($ppcp_documentation_link) . '" target="_blank">' . __('Read this documentation', 'wordpress-simple-paypal-shopping-cart') . '</a> ' . __('to learn how to set up and configure it.', 'wordpress-simple-paypal-shopping-cart');
		echo '</p>';
		echo '</div>';
		...
		<h3 class="nav-tab-wrapper">
			<a class="nav-tab <?php echo ($subtab == '' || $subtab == 'api-connection') ? 'nav-tab-active' : ''; ?>" href="<?php echo esc_url_raw($this->ppcp_connection_subtab_url); ?>"><?php _e('PayPal API Connection', 'wordpress-simple-paypal-shopping-cart'); ?></a>
			<a class="nav-tab <?php echo ($subtab == 'api-credentials') ? 'nav-tab-active' : ''; ?>" href="<?php echo esc_url_raw($this->ppcp_api_creds_subtab_url); ?>"><?php _e('API Credentials', 'wordpress-simple-paypal-shopping-cart'); ?></a>			
			<a class="nav-tab <?php echo ($subtab == 'button-appreance') ? 'nav-tab-active' : ''; ?>" href="<?php echo esc_url_raw($this->ppcp_button_subtab_link); ?>"><?php _e('Button Appearance', 'wordpress-simple-paypal-shopping-cart'); ?></a>
		</h3>
		<br />
		<?php

		// Handle PayPal ppcp checkout settings form submit
		if (isset($_POST['wpapp_ppcp_checkout_settings_submit']) && check_admin_referer('wpapp_ppcp_checkout_settings_submit_nonce')) {
        ...
        if (isset($_GET['wpapp_ppcp_after_onboarding'])){
            ...
            echo '<div class="notice notice-success"><p>' . esc_attr($onboarding_action_result) . '</p></div>';
        }

		if (isset($_GET['wpapp_ppcp_disconnect_production'])){
            //Verify nonce
            check_admin_referer( 'wpapp_ac_disconnect_nonce_production' );
            ...
            echo '<div class="notice notice-success"><p>' . esc_attr($disconnect_action_result) . '</p></div>';
        }

        if (isset($_GET['wpapp_ppcp_disconnect_sandbox'])){
            //Verify nonce
            check_admin_referer( 'wpapp_ac_disconnect_nonce_sandbox' );
		...
		if (isset($_POST['wpapp_ppcp_api_credentials_submit']) && check_admin_referer('wpapp_ppcp_api_credentials_submit_nonce')) {
		...
		if (isset($_POST['wpapp_button_appearance_submit']) && check_admin_referer('wpapp_button_appearance_submit_nonce', 'wpapp_button_appearance_submit_nonce')) {
		...
		if (isset($_GET['wpapp_ppcp_delete_cache'])) {
			check_admin_referer('wpapp_ppcp_delete_cache');
			/**
		...
		do_action('wpapp_ppcp_settings_menu_tab_end');

		wpapp_settings_menu_footer();
	}
					...
					<tbody>
						<tr>
							...
							<td>
								...
								$delete_cache_url = add_query_arg('wpapp_ppcp_delete_cache', 1, $delete_cache_url);
								$delete_cache_url_nonced = add_query_arg('_wpnonce', wp_create_nonce('wpapp_ppcp_delete_cache'), $delete_cache_url);
								echo '<p><a class="button wpapp-paypal-delete-cache-btn" href="' . esc_url_raw($delete_cache_url_nonced) . '">' . __('Delete Token Cache', 'wordpress-simple-paypal-shopping-cart') . '</a></p>';
			...
			<h2><?php _e("PayPal API Credentials for PPCP", 'wordpress-simple-paypal-shopping-cart'); ?></h3>
				<div class="inside">
					...
					<form action="" method="POST">
						...
						<input type="submit" name="wpapp_ppcp_api_credentials_submit" class="button-primary" value="<?php _e('Save Changes', 'wordpress-simple-paypal-shopping-cart'); ?>" />
						<?php wp_nonce_field('wpapp_ppcp_api_credentials_submit_nonce'); ?>
					</form>
				</div>
		</div>
		<?php
		...
		<div class="postbox">
			...
			<div class="inside">
				...
				<form id="ppcp_button_config_form" method="post">
					...
					<?php wp_nonce_field('wpapp_button_appearance_submit_nonce', 'wpapp_button_appearance_submit_nonce') ?>
					<input type="submit" name="wpapp_button_appearance_submit" class="button-primary" value="<?php _e('Save Changes', 'wordpress-simple-paypal-shopping-cart'); ?>">
				</form>
			</div>
		</div>
	<?php
	...
	public function handle_paypal_ppcp_connection_settings()
	{
		...
		<table class="form-table" role="presentation">
			<tbody>
				<tr>
					...
					<td>
						...
						if ($live_account_connection_status == 'connected') {
							...
							echo '<div class="wpapp-paypal-live-account-status"><span class="dashicons dashicons-yes" style="color:green;"></span>&nbsp;';
							...
							echo '<div class="wpapp-paypal-live-account-status"><span class="dashicons dashicons-no" style="color: red;"></span>&nbsp;';
				...
				<?php 
				$sandbox_enabled = defined('WPAPP_ENABLE_SANDBOX') ? WPAPP_ENABLE_SANDBOX : false;
				if($sandbox_enabled){
				?>
				<tr>
					...
					<td>
						...
						if ($sandbox_account_connection_status == 'connected') {
							...
							echo '<div class="wpapp-paypal-sandbox-account-status"><span class="dashicons dashicons-yes" style="color:green;"></span>&nbsp;';
							...
							echo '<div class="wpapp-paypal-sandbox-account-status"><span class="dashicons dashicons-no" style="color: red;"></span>&nbsp;';
...
function wpapp_settings_menu_footer()
{
    ?>
	<style>
		.wpapp-yellow-box{
