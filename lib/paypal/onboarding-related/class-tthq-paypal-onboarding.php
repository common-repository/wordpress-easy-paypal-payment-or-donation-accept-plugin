...
namespace TTHQ\WPAPP\Lib\PayPal\Onboarding;

use TTHQ\WPAPP\Lib\PayPal\PayPal_Main;
use TTHQ\WPAPP\Lib\PayPal\PayPal_Utility_Functions;
...
class PayPal_PPCP_Onboarding {
	...
	*/
	public static $account_connect_string = 'wpapp_ppcp_account_connect';
	...
	public static function generate_return_url_after_onboarding( $environment_mode = 'production' ){
		...
		* REPLACE: plugin prefix across different plugins.
		*/
		$query_args['wpapp_ppcp_after_onboarding'] = '1';
	...
	*/
	public function output_sandbox_onboarding_link_code() {
		...
		?>
		<script>
			function wpapp_ppcp_onboarded_callback_sandbox(authCode, sharedId) {
				console.log('wpapp PayPal Sandbox Onboarded-Callback');
				...
				formData.append('action', 'wpapp_handle_onboarded_callback_data');
		...
		<a class="button button-primary direct" target="_blank"
			data-paypal-onboard-complete="wpapp_ppcp_onboarded_callback_sandbox"
	...
	*/
	public function output_sandbox_ac_disconnect_link(){
		$disonnect_link_path = PayPal_Main::$pp_api_connection_settings_menu_page . '&wpapp_ppcp_disconnect_sandbox=1';
		$sandbox_disconnect_url = admin_url($disonnect_link_path);
		$ac_disconnect_nonce = wp_create_nonce('wpapp_ac_disconnect_nonce_sandbox');
	...
	*/
	public function output_production_onboarding_link_code() {
		...
		?>
		<script>
			function wpapp_ppcp_onboarded_callback_production(authCode, sharedId) {
				console.log('wpapp PayPal Production Onboarded-Callback');
				//Send the authCode and sharedId to your server and do the next steps.
				//You can use the sellerNonce to identify the user.				
		...
		<a class="button button-primary direct" target="_blank"
			data-paypal-onboard-complete="wpapp_ppcp_onboarded_callback_production"
	...
	*/
	public function output_production_ac_disconnect_link(){
		$disonnect_link_path = PayPal_Main::$pp_api_connection_settings_menu_page . '&wpapp_ppcp_disconnect_production=1';
		$disconnect_url = admin_url($disonnect_link_path);
		$ac_disconnect_nonce = wp_create_nonce('wpapp_ac_disconnect_nonce_production');
