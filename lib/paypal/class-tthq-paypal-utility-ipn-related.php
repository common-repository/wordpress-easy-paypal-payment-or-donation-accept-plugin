	...
	public static function validate_buy_now_checkout_txn_data( $data, $txn_data ) {
		...
		if( $order_details !== false ){
			...
			if ( strtolower($status) != strtolower('COMPLETED') ) {
				...
				$validation_error_msg = 'Validation Error! The transaction status is not completed yet. PayPal Capture ID: ' . $capture_id . ', Capture Status: ' . $status;
			...
			//TODO - Get the amount from the transient data.
			$payment_amount_expected = 0;
			if( floatval($amount) < floatval($payment_amount_expected) ){
				//The amount does not match.
				$validation_error_msg = 'Validation Error! The payment amount does not match. Amount Received: ' . $amount . ', Amount Expected: ' . $payment_amount_expected;
				PayPal_Utility_Functions::log( $validation_error_msg, false );
				return $validation_error_msg;
			}
			...
			//TODO - Get the currency from the transient data.
			$currency_expected = 'USD';
			if( $currency != $currency_expected ){
				//The currency does not match.
				$validation_error_msg = 'Validation Error! The payment currency does not match. Currency Received: ' . $currency . ', Currency Expected: ' . $currency_expected;
				PayPal_Utility_Functions::log( $validation_error_msg, false );
				return $validation_error_msg;
			}
