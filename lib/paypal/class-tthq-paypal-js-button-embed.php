...
class PayPal_JS_Button_Embed {
	...
	*/
    public $button_id_prefix = 'wpapp_paypal_button_';
	...
	public function generate_paypal_js_sdk_args( $args = array()){
		...
		if( empty($sdk_args['client-id']) ){
			//Client ID is required. Add a log entry.
		 ...
		 * * For example 'generate_paypal_js_sdk_args' is actually 'wpapp_generate_paypal_js_sdk_args' for simple cart plugin.
		 */ 
	...
	public function load_paypal_sdk_for_subscriptions() {
		...
		<script type="text/javascript">
			...
			* REPLACE: plugin prefix across different plugins.
			*/
			script_sub.setAttribute( 'data-namespace', 'wpapp_paypal_subscriptions' );//Use a different namespace for the subscription buttons.
			script_sub.async = true;
			script_sub.src = '<?php echo esc_url_raw( $script_url ); ?>';
			script_sub.onload = function() {
				document.dispatchEvent(new Event('wpapp_paypal_sdk_subscriptions_loaded'));//REPLACE: plugin prefix across different plugins.
			};
