...
class PayPal_Request_API_Injector {
        ...
        */
        public function create_paypal_order_by_url_and_args( $data, $additional_args = array() ){
            ...
            $shipping_amt = isset($data['shipping_amt']) ? $data['shipping_amt'] : 0;
            $tax_amt = isset($data['tax_amt']) ? $data['tax_amt'] : 0;
            $currency_code = isset($data['currency']) ? $data['currency'] : 'USD';
            $description = isset($data['description']) ? $data['description'] : '';

            $item_name = isset($data['item_name']) ? $data['item_name'] : '';
            $quantity = isset($data['quantity']) ? $data['quantity'] : 1;
            $item_amount = isset($data['item_amount']) ? $data['item_amount'] : 0;
            ...
            $order_data = [
                ...
                "purchase_units" => [
                    [
                        "amount" => [
                            "value" => number_format($grand_total,2,'.',''), /* The grand total that will be charged for the transaction. */
                            "currency_code" => $currency_code,
                            "breakdown" => [
                                "item_total" => [
                                    "currency_code" => $currency_code,
                                    "value" => number_format($sub_total,2,'.',''), /* We can break down the total amount into item_total, tax_total, shipping etc */
                                ]
                            ]
                        ],
                        "items" => [
                            [
                                ...
                                "quantity" => $quantity,
                                "unit_amount" => [
                                    "value" => number_format($item_amount,2,'.',''), /* Individual item amount. */
                                    "currency_code" => $currency_code,
                                ]
                            ]
                        ],
            ...
            if( isset($shipping_amt) && $shipping_amt > 0){
                $order_data['purchase_units'][0]['amount']['breakdown']['shipping'] = [
                    "currency_code" => $currency_code,
                    "value" => number_format($shipping_amt,2,'.',''), 
                ];
            }
